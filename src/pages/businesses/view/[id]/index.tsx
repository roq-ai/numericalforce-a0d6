...
function BusinessViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<BusinessInterface>(
    ...
    () =>
      getBusinessById(id, {
        relations: ['user', 'expense', 'financial_report', 'invoice'],
      }),
  );
              ...
              {hasAccess('expense', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/expenses/create?business_id=${data?.id}`}>
              ...
              {hasAccess('financial_report', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/financial-reports/create?business_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                    ...
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.financial_report?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/financial-reports/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>
                              {hasAccess('financial_report', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/financial-reports/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('financial_report', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    financial_reportHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
